...
public class OrderVendorDao {
	 ...
	 public OrderVendorDao(Session sess){
		 this.session = sess;
	 }
	
	public List<TOrder> getOrderList(int vendorId,String flag) {//获取账户列表
		SQLQuery q;
        ...
        String sql="select t.order_num,t.`order_menuinfo_id`,t.`user_id`,t.box_id,t.`box_price`,t.`order_notice`,t.`order_isFirst`" +
                ...
                " from t_order t,t_user tuser,`t_box` tbox,`t_menuinfo` tmenu,`t_dispatching` tdispatch where " +
                "t.`box_id`=tbox.`box_id` and t.`order_dispatching_id`=tdispatch.`dispatching_id` and t.`user_id`=tuser.`user_id` " +
                "and t.`order_menuinfo_id`=tmenu.`menuinfo_id` and tmenu.`vendor_id`=?";
        if(flag.equals("today")){
            sql += " and Date(t.`order_orderTime`)=Date(now())";
...
/*        q = session.createSQLQuery("select t.order_num,t.`order_menuinfo_id`,t.`user_id`,t.box_id,t.`box_price`,t.`order_notice`,t.`order_isFirst`" +
                ...
                " from t_order t,t_user tuser,`t_box` tbox,`t_menuinfo` tmenu,`t_dispatching` tdispatch where " +
                "t.`box_id`=tbox.`box_id` and t.`order_dispatching_id`=tdispatch.`dispatching_id` and t.`user_id`=tuser.`user_id` " +
                "and t.`order_menuinfo_id`=tmenu.`menuinfo_id` and tmenu.`vendor_id`=? and Date(t.`order_orderTime`)=Date(now())");*/
        q.setParameter(0,vendorId);
		List l = q.list();
